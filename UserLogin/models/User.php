...
class UserModel extends Db
{
    public function getAllDate(){
        $sql = self::$connection->prepare("SELECT DISTINCT `date` FROM autocheck");
        return $this->select($sql);
    }
    public function getUserCheckedByDate($date){
        $sql = self::$connection->prepare("SELECT * FROM autocheck WHERE date=?");
        $sql->bind_param('s',$date);
        return $this->select($sql);
    }
    public function get(){
        $sql = self::$connection->prepare("SELECT * FROM autocheck");
        return $this->select($sql);
    }
     //Lấy thông tin user từ user_id
     public function getUserByMSSV($mssv){
        $sql = self::$connection->prepare("SELECT * FROM users WHERE mssv=? AND user_type='user'");
        $sql->bind_param('s',$mssv);
        return $this->select($sql)[0];
    }
    //Lấy thông tin user từ user_id
    public function getUserByMSSVS($mssv){
        $date = date("Y/m/d");
        $sql = self::$connection->prepare("SELECT * FROM autocheck WHERE mssv=? AND date=?");
        $sql->bind_param('ss',$mssv,$date);
        return $this->select($sql);
    }
    //Lấy thông tin user từ user_id
    public function getUserByUserId($custom_id){
        $date = date("Y/m/d");
        $sql = self::$connection->prepare("SELECT * FROM autocheck WHERE user_id=? AND date=?");
        $sql->bind_param('ss',$custom_id,$date);
        return $this->select($sql);
    }
    public function Insert($mssv){
        
        $sql = self::$connection->prepare("INSERT INTO `autocheck`(`mssv`) VALUES (?)");
        $sql->bind_param('s',$mssv);
        $sql->execute();
        return true;
    }
    ...
    public function getAllUser(){
        $sql = self::$connection->prepare("SELECT * FROM users WHERE user_type='user'");
        return $this->select($sql);
    }
    //lấy tất cả user
    ...
    //Trả về kết quả Login Admin
    public function getLoginAdmin($username,$password)
    {
        $pass_md5 = md5($password);
        $sql = self::$connection->prepare("SELECT * FROM users WHERE username=? AND password=? AND user_type='admin'");
