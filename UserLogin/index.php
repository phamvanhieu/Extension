<?php
    ...
    if (isset($_POST['edit_user'])) {
        if ($_FILES['img_user']['name']) {
            ...
            switch ($type_img) {
                ...
                case 'application/vnd.openxmlformats-officedocument.wordprocessingml.document': //file word
                    if($size_img < 2000000){
                        ...
                        $email = $_POST['email'];
                        $users = $userModel->getUserByCustomId($_SESSION['custom_id']);
                        var_dump($users);
                        exit;
            ...
            $users = $userModel->getUserByCustomId($_SESSION['custom_id']);
            $userModel->setUser($_SESSION['custom_id'],$fullname,$email);
            echo "Version: ". $user_version . " va " . $users['version'];
            exit;
            // if ($users['version'] == $user_version) {
            //     array_push($errors, "Da sua!!!");
            //     $user = $userModel->setUser($_SESSION['custom_id'],$fullname,$email);
            // }else{
            //     array_push($errors, "trùng nhau!!!");
            // }
            // header("location: index.php?edit=success");
        }
